...
void setup() {
  Serial.begin(9600);
  if (!rf22.init()){Serial.println("RF22 init failed");} //begin the rf22 
...
void loop() {
  ...
  // TODO: read coordinates from gps. Send them on phase_2 for the network to be able to map the potholes.
  if (ok){
  //if (1){
    phase_2_A();
    lo = phase_2_B();
...
void phase_1_A(){
  Serial.println("1_A");
  if (!rf22.setFrequency(freq_up)){Serial.println("setFrequency failed in phase_1_A");}
  uint8_t data_send[RF22_ROUTER_MAX_MESSAGE_LEN];
  char data[] = "hello, i am inside your range";
  memcpy(data_send,data,RF22_ROUTER_MAX_MESSAGE_LEN);
  if (rf22.sendtoWait(data_send,sizeof(data_send),RF22_BROADCAST_ADDRESS) != RF22_ROUTER_ERROR_NONE){Serial.println("sendtoWait failed in phase_1_A (for the first message)");}
  rf22.waitPacketSent();
  Serial.println("i sent the insert message, now i will send my id");
  sprintf(data, "%d", id);
  memcpy(data_send,data,RF22_ROUTER_MAX_MESSAGE_LEN);
  delay(50);
  if (rf22.sendtoWait(data,sizeof(data),RF22_BROADCAST_ADDRESS) != RF22_ROUTER_ERROR_NONE){Serial.println("sendtoWait failed in phase_1_A (for the id)");}
  rf22.waitPacketSent();
...
int phase_1_B(){
  ...
  else{Serial.println("Receive failed in phase_1_B");}
  return ret;
}
...
void phase_2_A(){
  Serial.println("2_A");
  if (!rf22.setFrequency(freq_up)){Serial.println("setFrequency failed in phase_2_A");}
  char data[] = "i have something for you! A new pothole";
  uint8_t data_send[RF22_ROUTER_MAX_MESSAGE_LEN];
  memcpy(data_send,data,RF22_ROUTER_MAX_MESSAGE_LEN);
  if (rf22.sendtoWait(data_send,sizeof(data_send),RF22_BROADCAST_ADDRESS) != RF22_ROUTER_ERROR_NONE){Serial.println("sendtoWait failed in phase_2_A");}
  rf22.waitPacketSent();
