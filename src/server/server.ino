...
void loop() {
  if (!rf22.setFrequency(freq_up)) {
    Serial.println("setFrequency failed in loop");
  }
  int id_temp = 0; // To save the id of the client
  int temp2 = 0; // 1 if i sent before the same message to a client, 2 if i dont
                 // have an information, 0 otherwise
  int lo = 0; // 1 if the client send an information, 0 otherwise

  // TODO: the id_temp, temp2 and lo, must be vectors for the network to serve
  ...
  if (rf22.recvfromAckTimeout(buf, &len, max_receice_time)) {
    if (strcmp((char *)buf, ack1) == 0) {
      ...
      Serial.println("Something wrong happened here!");
    }
    if (lo) {
      phase_2_B();
    }
...
void phase_1_B(int temp2, int gps_flag) {
  ...
  case 0:
    ...
    if (rf22.sendtoWait(data_send3, sizeof(data_send3),
                        RF22_BROADCAST_ADDRESS) != RF22_ROUTER_ERROR_NONE) {
      Serial.println("sendtoWait failed in phase_1_B (gia to alt)");
    }
    rf22.waitPacketSent();
    Serial.println("i sent him the flag of the gps");
  case 1:
    if (rf22.sendtoWait(data_send1, sizeof(data_send1),
                        RF22_BROADCAST_ADDRESS) != RF22_ROUTER_ERROR_NONE) {
      Serial.println(
          "sendtoWait failed in phase_1_B (gia to minima pou ton exw ksana)");
    }
    rf22.waitPacketSent();
    Serial.println("i allready sent him once");
  case 2:
    if (rf22.sendtoWait(data_send2, sizeof(data_send2),
                        RF22_BROADCAST_ADDRESS) != RF22_ROUTER_ERROR_NONE) {
      Serial.println(
          "sendtoWait failed in phase_1_B (gia to minima pou den exw data)");
    }
    rf22.waitPacketSent();
...
void phase_2_B() {
  ...
  if (rf22.sendtoWait(data_send, sizeof(data_send), RF22_BROADCAST_ADDRESS) !=
    ...
    Serial.println("sendtoWait failed in phase_2_B ");
  }
  rf22.waitPacketSent();
}
